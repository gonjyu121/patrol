...
; v7: bounded recovery with a visible, non-input status overlay. No synthetic player activity or Falix power actions.
; Runtime settings/images/logs stay on the streaming PC, not on the shared drive.
global dataDir := A_AppData "\PatrolReconnectV7"
global gameDir := ""
...
Tell(message) {
    ...
    TrayTip(message, "Patrol Reconnect v7")
}
...
SelfTest() {
    ...
    global dataDir
    dataDir := A_Temp "\PatrolReconnectV7-test-" A_TickCount
    DirCreate(dataDir)
    try {
        PrepareTemplates()
