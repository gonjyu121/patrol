...
; v8: detect the disconnect screen directly in addition to log events. No synthetic player activity or Falix power actions.
; Runtime settings/images/logs stay on the streaming PC, not on the shared drive.
global dataDir := A_AppData "\PatrolReconnectV8"
global gameDir := ""
...
MsgBox("再接続 v8：画像登録・ログフォルダ指定は不要です。`n旧AHKは終了してください。`n`nMinecraftを前面にしてF6: 監視開始`n監視中は左上の表示が点滅し、15秒ごとに小さな視点・左右移動の演出を行います。`nF7: 全停止（手動切断前にも押す）`nF8: 終了`n`n撮影時と同じGUIスケールで使用してください。`nFalixの起動操作は行いません。")
...
Tell(message) {
    ...
    TrayTip(message, "Patrol Reconnect v8")
}
...
SelfTest() {
    ...
    global dataDir
    dataDir := A_Temp "\PatrolReconnectV8-test-" A_TickCount
    DirCreate(dataDir)
    try {
        PrepareTemplates()
    ...
    FileAppend("PASS: detection, argument, and direct-screen trigger checks plus 2 template BMP checks; no GUI/input/server actions.`n", "*")
}
