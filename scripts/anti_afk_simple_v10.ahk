...
; v10: tolerate the verified hover-state brightness difference on the back button.
; Runtime settings/images/logs stay on the streaming PC, not on the shared drive.
global dataDir := A_AppData "\PatrolReconnectV10"
global gameDir := ""
...
MsgBox("再接続 v10：画像登録・ログフォルダ指定は不要です。`n旧AHKは終了してください。`n`nMinecraftを前面にしてF6: 監視開始`n監視中は左上の表示が点滅し、15秒ごとに小さな視点・左右移動の演出を行います。`nF7: 全停止（手動切断前にも押す）`nF8: 終了`n`n撮影時と同じGUIスケールで使用してください。`nFalixの起動操作は行いません。")
...
Tell(message) {
    ...
    TrayTip(message, "Patrol Reconnect v10")
}
...
UpdateStatusOverlay() {
    global armed, targetHwnd, statusGui, statusText, pulse, phase, used
    if !IsObject(statusGui)
        return
    ...
    pulse := !pulse
    marker := pulse ? "●" : "○"
    label := phase = "back" || phase = "server" ? "再接続準備中"
        : phase = "settle" ? "再接続確認中"
        : "再接続監視中"
...
SelfTest() {
    ...
    global dataDir
    dataDir := A_Temp "\PatrolReconnectV10-test-" A_TickCount
    DirCreate(dataDir)
    try {
        PrepareTemplates()
    ...
    FileAppend("PASS: direct-screen, hover-tolerance, server-safety, log, argument, and 2 template BMP checks; no GUI/input/server actions.`n", "*")
}
