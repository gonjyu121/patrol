...
; v9: direct screen detection, bounded reconnect recovery, and disclosed presentation motion.
; Runtime settings/images/logs stay on the streaming PC, not on the shared drive.
global dataDir := A_AppData "\PatrolReconnectV9"
global gameDir := ""
...
MsgBox("再接続 v9：画像登録・ログフォルダ指定は不要です。`n旧AHKは終了してください。`n`nMinecraftを前面にしてF6: 監視開始`n監視中は左上の表示が点滅し、15秒ごとに小さな視点・左右移動の演出を行います。`nF7: 全停止（手動切断前にも押す）`nF8: 終了`n`n撮影時と同じGUIスケールで使用してください。`nFalixの起動操作は行いません。")
...
Tell(message) {
    ...
    TrayTip(message, "Patrol Reconnect v9")
}
...
SelfTest() {
    ...
    global dataDir
    dataDir := A_Temp "\PatrolReconnectV9-test-" A_TickCount
    DirCreate(dataDir)
    try {
        PrepareTemplates()
    ...
    FileAppend("PASS: log, exclusion, argument, and real direct-screen trigger checks plus 2 template BMP checks; no GUI/input/server actions.`n", "*")
}
