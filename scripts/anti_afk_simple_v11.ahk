...
; v11: detect Falix's offline lobby and clearly hand server startup back to the user.
; Runtime settings/images/logs stay on the streaming PC, not on the shared drive.
global dataDir := A_AppData "\PatrolReconnectV11"
global gameDir := ""
global falixDashboardUrl := "https://client.falixnodes.net/"
global templateSizes := Map("back", [276, 32], "server", [318, 32])
global templateVariation := Map("back", 60, "server", 15)
...
MsgBox("再接続 v11：画像登録・ログフォルダ指定は不要です。`n旧AHKは終了してください。`n`nMinecraftを前面にしてF6: 監視開始`n監視中は左上の表示が点滅し、15秒ごとに小さな視点・左右移動の演出を行います。`nF7: 全停止（手動切断前にも押す）`nF8: 終了`nF9: Falix管理画面を開く`n`nFalixの停止ロビーを検知すると演出を停止して通知します。サーバーの起動と本サーバーへの再接続は手動で行い、接続後にF6を押してください。")

F6::Arm()
F7::Halt("手動停止")
F8::ExitApp()
F9::OpenFalixDashboard()

IsFailure(text) {
    return !!RegExMatch(text, "i)(Connection reset|Connection timed out|Read timed out|Timed out|Connection lost|lost connection|SocketException|Connection closed|Server closed|End of stream)")
}
...
IsOfflineLobby(text) {
    ; Match only Falix's generic lobby wording. Never retain or log the
    ; server-specific startup URL that can appear on the following line.
    return InStr(text, "This server is currently offline")
        || InStr(text, "Start it by clicking the link below")
}

ShouldStartRecovery(logFailure, disconnectVisible) {
    return logFailure || disconnectVisible
}

Tell(message) {
    global dataDir
    try FileAppend(FormatTime(, "yyyy-MM-dd HH:mm:ss") " " message "`n", dataDir "\reconnect.log", "UTF-8")
    TrayTip(message, "Patrol Reconnect v11")
}

OpenFalixDashboard() {
    global falixDashboardUrl
    try {
        Run(falixDashboardUrl)
        Tell("Falix管理画面を開きました。起動操作は手動で行ってください")
    } catch as e {
        Tell("Falix管理画面を開けません: " e.Message)
    }
}
...
EnterOfflineLobby() {
    ...
    SoundBeep(880, 180)
    SoundBeep(660, 260)
    Tell("Falix停止ロビーを検知。F9で管理画面を開き、手動起動・再接続後にF6")
}

Halt(reason) {
    global armed, phase, epoch, statusGui
    armed := false
    phase := "idle"
    epoch += 1
    Send("{a up}{d up}")
...
UpdateStatusOverlay() {
    global armed, targetHwnd, statusGui, statusText, statusModeText, pulse, phase, used
    if !IsObject(statusGui)
        return
    if (!armed || !targetHwnd || !WinExist("ahk_id " targetHwnd) || WinExist("A") != targetHwnd) {
        statusGui.Hide()
        return
    }
    pulse := !pulse
    marker := pulse ? "●" : "○"
    label := phase = "lobby" ? "Falix停止ロビーを検知"
        : phase = "back" || phase = "server" ? "再接続準備中"
        : phase = "settle" ? "再接続確認中"
        : "再接続監視中"
    statusText.Text := marker " " label "  " used "/3"
    statusModeText.Text := phase = "lobby"
        ? "F9: 管理画面 / 手動再接続後にF6"
        : "自動演出中"
    try {
...
IsGameDir(path) {
    return path != "" && FileExist(path "\logs\latest.log")
}
...
DetectGameDir(hwnd) {
    ...
    for process in service.ExecQuery("SELECT CommandLine FROM Win32_Process WHERE ProcessId=" pid) {
        ...
        if result {
            ...
            if !IsGameDir(result)
                throw Error("起動引数のゲームフォルダにlogs\latest.logがありません")
            return result
        }
        ; The official launcher may omit --gameDir for its standard directory.
        standard := A_AppData "\.minecraft"
        if IsGameDir(standard)
            return standard
        throw Error("--gameDirがなく、標準の%AppData%\.minecraftにもログがありません")
    }
...
Tick() {
    ...
    try {
        ...
        if phase = "lobby"
            return
        if phase = "idle" {
            detectedByLog := IsFailure(text)
            detectedByScreen := false
            ; The client can show this menu without appending a matching log
            ; line. Check the known button only in the focused target window.
            if (!detectedByLog && MinecraftActive() = targetHwnd)
                detectedByScreen := FindTemplate("back", &screenX, &screenY)
            if !ShouldStartRecovery(detectedByLog, detectedByScreen)
                return
            if used >= 3 {
            ...
            if detectedByScreen
                Tell("切断画面を直接検知。10秒後に再確認します（F7で取消）")
            else
...
FindTemplate(name, &x, &y) {
    global targetHwnd, dataDir, templateVariation
    WinGetClientPos(&left, &top, &width, &height, "ahk_id " targetHwnd)
    variation := templateVariation[name]
    return ImageSearch(&x, &y, left, top, left + width - 1, top + height - 1, "*" variation " " dataDir "\" name ".bmp")
}
...
SelfTest() {
    ...
    for text in ["[CHAT] This server is currently offline", "Start it by clicking the link below:"] {
        if !IsOfflineLobby(text)
            throw Error("Offline lobby detection: " text)
    }
    for text in ["Joining the game", "Server closed", "normal chat"] {
        if IsOfflineLobby(text)
            throw Error("Offline lobby false positive: " text)
    }
    if !IsFailure("Server closed")
        throw Error("Server closed detection")
    if !ShouldStartRecovery(false, true)
        throw Error("Direct disconnect screen trigger")
    if !ShouldStartRecovery(true, false)
        throw Error("Log recovery trigger")
    if ShouldStartRecovery(false, false)
        throw Error("False recovery trigger")
    global templateVariation
    if templateVariation["back"] < 55
        throw Error("Back-button variation is below the verified hover difference")
    if templateVariation["server"] != 15
        throw Error("Server-template variation was unintentionally relaxed")
    ...
    if !IsGameDir(A_Temp)
        ...
        throw Error("Invalid game directory accepted")
    global dataDir
    dataDir := A_Temp "\PatrolReconnectV11-test-" A_TickCount
    DirCreate(dataDir)
    try {
        PrepareTemplates()
    ...
    FileAppend("PASS: offline-lobby, direct-screen, hover-tolerance, server-safety, log, argument, and 2 template BMP checks; no GUI/input/server actions.`n", "*")
}
