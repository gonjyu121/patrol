...
; v6: bounded recovery only. No synthetic player activity or Falix power actions.
; Runtime settings/images/logs stay on the streaming PC, not on the shared drive.
global dataDir := A_AppData "\PatrolReconnectV6"
global gameDir := ""
...
SetTimer(Tick, 1000)
MsgBox("再接続 v6：画像登録・ログフォルダ指定は不要です。`n旧AHKは終了してください。`n`nMinecraftを前面にしてF6: 監視開始`nF7: 全停止（手動切断前にも押す）`nF8: 終了`n`n撮影時と同じGUIスケールで使用してください。`n定期キー送信・Falixの起動は行いません。")
...
Tell(message) {
    ...
    TrayTip(message, "Patrol Reconnect v6")
}
...
SelfTest() {
    ...
    global dataDir
    dataDir := A_Temp "\PatrolReconnectV6-test-" A_TickCount
    DirCreate(dataDir)
    try {
        PrepareTemplates()
